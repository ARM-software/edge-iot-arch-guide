...
`FW_AB_REJECTED`: The running firmware has been rejected.
`FW_AB_SUCCESS`: A background firmware update completed successfully.
`FW_AB_ERROR_*`: A background firmware update failed with an error.
...
**Note**: If the update agent does not have access to the metadata at runtime, on the next reboot it
